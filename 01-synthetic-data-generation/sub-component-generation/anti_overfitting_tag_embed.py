...
# -*- coding: utf-8 -*-
"""
Adaptive tag placement utilities for synthetic training samples.

The module adds controlled real-world appearance variation to improve detector generalization.
"""
...
def blend_multiply(tag_f, roi_f, alpha):
    """Multiply blending; darker tones become more dominant."""
    mult = (tag_f / 255.0) * (roi_f / 255.0) * 255.0
    return alpha * mult + (1.0 - alpha) * roi_f


def blend_screen(tag_f, roi_f, alpha):
    """Screen blending; brighter tones become more dominant."""
...
def blend_overlay(tag_f, roi_f, alpha):
    """Overlay blending; increases local contrast."""
    mask = roi_f < 128
...
def _resolve_pipeline_flags(pipeline_mode: str):
    ...
    if mode == PIPELINE_MODE_FULL:
        ...
        flags.update({
            # Naive same-scale: raw copy-paste baseline.
            # Advanced adaptations are disabled, including mask and alpha handling.
        })
    elif mode == PIPELINE_MODE_NO_BG_AWARE:
        flags.update({
            # Background-aware block is disabled; basic masked placement is retained.
            "use_appearance_realism": True,
            "use_alpha_builder": True,
            "use_mask_gate": True,
            "use_occlusion": True,
...
def add_opaque_background_to_tag(tag_bgr, mask_crop, rng):
    """
    Add a realistic opaque paper-like background to transparent tag crops.

    The mask is expanded so that the visible tag area remains fully opaque.
    """
    # Select a realistic paper-like background color
    ...
    # Dilate the mask so tag edges are fully covered
    ...
    # Smooth the mask with a light blur
    ...
    # Keep the background over the whole crop, not only inside the tag region
    result = tag_bgr.astype(np.float32) * mask_3ch + background * (1.0 - mask_3ch)
    
    # Make the full crop opaque so the edges remain paper-colored
...
def validate_tag_crop_quality(tag_crop, mask_crop, min_area_ratio=0.25, max_area_ratio=0.95):
    ...
    Args:
        ...
        mask_crop: Tag mask.
        min_area_ratio: Minimum mask area ratio used to reject very small tags.
        max_area_ratio: Maximum mask area ratio.
    ...
    # 5. Aspect-ratio check with a wider tolerance
    aspect_ratio = max(w_box, h_box) / (min(w_box, h_box) + 1e-6)
    
    # Very long or thin crops (>6:1) are usually unreliable
    if aspect_ratio > 6.0:
    ...
    # 6. Fragmentation check with a wider tolerance
    if len(contours) > 2:  # Check when there are more than two components
...
def compute_tight_bbox_from_mask(mask_crop, padding_ratio=0.05):
    ...
    Args:
        ...
        padding_ratio: Padding ratio added around the detected mask.

    Returns:
        (x_min, y_min, x_max, y_max) in unnormalized pixel coordinates.
    """
    ...
    # Add light padding around the box
...
def add_bbox_augmentation(bbox, scene_shape, rng, augment_prob=0.3):
    """
    Apply a small random perturbation to the bounding box.

    Args:
        bbox: (x0, y0, x1, y1) in scene coordinates.
        scene_shape: (H, W, C).
        rng: Random generator.
        augment_prob: Probability of applying the perturbation.
    ...
    # Apply to randomly selected edges
    if rng.random() < 0.5:
...
# ============================================
# 1. REAL-WORLD APPEARANCE VARIATIONS
# ============================================

def apply_realistic_degradation(tag_bgr, rng, severity="random"):
    """
    Apply realistic tag degradations such as fading, dirt, scratches, and stains.
    """
    if severity == "random":
    ...
    if rng.random() < 0.4:
        ...
        dirt_noise = (dirt_noise + 1.0) / 2.0  # Normalize to the 0-1 range
...
def apply_environmental_lighting(tag_bgr, scene_roi, rng, strength_range=(0.3, 0.8)):
    """
    Adapt the tag appearance to the local environmental lighting.
    """
...
def add_realistic_occlusion(scene_roi, tag_mask, rng):
    """
    Apply conservative occlusion effects near tag edges.
    """
    h, w = tag_mask.shape[:2]
    occlusion_mask = np.ones((h, w), dtype=np.float32)
    occlusion_applied = False
    
    # Use a low occlusion probability and keep the effect conservative
    if rng.random() < 0.25:  # Apply with 25% probability
        edge_type = rng.choice(["corner", "side"])  # Irregular masks are not used
...
def add_perspective_depth_cues(tag_bgr, mask, perspective_angle, rng):
    """
    Add perspective depth cues by softening and darkening distant regions.
    """
    h, w = tag_bgr.shape[:2]
    
    # Depth gradient based on the perspective direction
    ...
    if blur_strength > 20:  # Only for strong perspective angles
        ...
        grad_levels = np.clip((1.0 - gradient) * 3, 0, 3)  # 0-3 range
    ...
    # Atmospheric perspective for distant regions
    darkness = gradient * 0.85 + 0.15  # 0.15-1.0 range
...
def adaptive_blend_selection(tag_prof, roi_prof, rng):
    """
    Select a blending mode using the tag and ROI photometric profiles.
    """
    ...
    # Weight suitable blending modes
    if brightness_diff < 30:
        # Similar brightness: prefer normal or soft light
        weights = {
            "normal": 0.50,
            "soft_light": 0.25,
            "overlay": 0.15,
        ...
        # Brighter tag: screen or lighten
        weights = {
            "normal": 0.35,
            "screen": 0.30,
            "lighten": 0.20,
    ...
    # ----------------------------------------------------
    # Matte paper-like tag: soft_light / lighten control
    # ----------------------------------------------------
    roi_bright = roi_prof.mean_L > 175  # Bright ROI: lighten may suppress the tag
    tag_paperish = (tag_prof.mean_L > 190) and (tag_prof.std_L < 18)  # Very bright with low contrast

    if roi_bright:
        # Lighten may preserve the bright ROI and weaken the tag
    ...
    if tag_paperish:
        # Very bright paper may reduce the effect of soft_light; lighten is also unnecessary
        if "lighten" in weights:
            weights["lighten"] *= 0.05
        if "soft_light" in weights:
            weights["soft_light"] *= 0.55

    # Safety fallback: keep normal if all weights approach zero
    ...
    # Add overlay or hard_light under high contrast
...
def anti_overfitting_overlay(scene_bgr,
    ...
    # 0. Quality check for the tag crop
    ...
    if x0 < 0 or y0 < 0 or x1 > w_s or y1 > h_s:
        raise ValueError("Tag extends outside the scene")
    ...
    # 1. PAPER-LIKE OPAQUE BACKGROUND
    ...
    # Mask gate: the warped mask can be preserved outside naive mode.
