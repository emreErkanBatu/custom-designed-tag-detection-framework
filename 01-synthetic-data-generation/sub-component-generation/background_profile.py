...
# -*- coding: utf-8 -*-
"""Patch profiling utilities for background-aware tag blending."""
...
@dataclass
class PatchProfile:
    """Photometric and quality profile of an image patch or ROI."""
    mean_L: float
    std_L: float
    mean_a: float
    mean_b: float
...
def _noise_score(gray: np.ndarray) -> float:
    """Simple noise estimate based on the standard deviation of gray minus Gaussian blur."""
...
def compute_patch_profile(bgr: np.ndarray,
    ...
    """
    bgr: BGR image.
    roi: (x0, y0, x1, y1) region in scene coordinates.
         If None, the whole image is profiled.
    """
