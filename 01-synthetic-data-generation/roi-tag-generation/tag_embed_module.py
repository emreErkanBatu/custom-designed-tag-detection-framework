...
# -----------------------------
# Alpha / edge blending variants + parameter debug
# -----------------------------
...
#     """
#     Distance method, kept to prevent excessive edge loss:
#     - keep alpha close to 1 inside core_px,
#     - reduce alpha only in the feather band after the core.
#     """
...
def _alpha_distance_feather(mask_u8: np.ndarray, rng: np.random.Generator):
    """
    Make the distance-based alpha method safe for small tags:
    - cap feather using dmax to reduce edge erosion,
    - floor core using dmax so the core does not become too small.
    """
    m = (mask_u8 > 0).astype(np.uint8)
    dist_in = cv2.distanceTransform(m, cv2.DIST_L2, 5).astype(np.float32)
    ...
    # Relative limits for small objects
    feather_cap = 0.22 * dmax          # Example: dmax=18.6 gives cap≈4.1, avoiding excessive feather values.
    core_floor  = 0.10 * dmax          # Example: dmax=18.6 gives floor≈1.86, avoiding an undersized core.

    feather_px = min(feather_px, feather_cap)
    core_px    = max(core_px, core_floor)

    # Extra safety: keep core + feather away from dmax.
    limit = 0.92 * dmax
...
def build_alpha(mask01: np.ndarray, rng: np.random.Generator, force_method: str = "random"):
    ...
    force_method:
      ...
      - "gauss" / "dist" / "morph" -> fixes one method
    """
    mask_u8 = (mask01.astype(np.uint8) * 255)
...
def soft_overlay_on_scene(scene_bgr,
    ...
    if x0 < 0 or y0 < 0 or x1 > w_s or y1 > h_s:
        raise ValueError("The tag extends outside the scene; adjust center_xy or size.")

    # Adapt the tag according to the ROI profile.
    roi_box = (x0, y0, x1, y1)
    tag_crop_bgr = adapt_tag_to_scene_roi(tag_crop_bgr, scene_bgr, roi_box, rng=rng)

    roi = scene[y0:y1, x0:x1]

    # Alpha method and parameter debug
    ...
    # Combine method and parameters into one string for main-script output.
